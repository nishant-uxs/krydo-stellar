...
\subsection{Current Technical Stack (\stellar{})}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}p{3.2cm} X@{}}
    ...
    Layer & Technology \\
    \midrule
    Smart contracts & \soroban{} Rust (\texttt{KrydoAuthority}, \texttt{KrydoCredentials}, \texttt{KrydoAudit}) \\
    Cryptography & Sigma protocols, Pedersen commitments, Fiat--Shamir (off-chain ZK) \\
    Backend & Node.js 20, Express, TypeScript, Zod, Firebase Firestore \\
    Frontend & React 18, Vite, shadcn/ui, TanStack Query \\
    Wallet & Freighter (\texttt{@stellar/freighter-api}) \\
    Auth & Sign-in-with-Stellar (SEP-53) + JWT \\
    Testing & 160+ unit tests (Vitest) \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{keybox}[\stellar{} Advantages for \krydo{}]
  \begin{enumerate}
    \item \textbf{Cost efficiency:} Sub-cent transaction fees enable high-volume verification anchoring without prohibitive per-credential costs.
...
\subsection{Why \stellar{} for Production}

\begin{table}[H]
  \centering
  \caption{\stellar{} mainnet fit for credential anchoring}
  \renewcommand{\arraystretch}{1.25}
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}p{3.5cm} X@{}}
    \toprule
    Dimension & \stellar{} Mainnet \\
    \midrule
    Avg.\ tx cost & \$0.00001--\$0.001 \\
    Contract language & Rust (\soroban{}) \\
    Finality & 3--5 sec \\
    Cross-border focus & Payments \& remittance native \\
    India fintech narrative & Financial inclusion alignment \\
    Residency support & Rise In / Garage mainnet sponsorship \\
    \bottomrule
  \end{tabularx}
\end{table}
...
% ═════════════════════════════════════════════════════════════════════════════
\section{Technical Roadmap on \stellar{}}

\subsection{Build Scope}

\krydo{} ships production-grade business logic, API design, ZK engine, and frontend UX on \stellar{}. The residency focuses on \textbf{Soroban contract hardening + Freighter wallet/auth + mainnet deployment}.

\begin{table}[H]
  \centering
  \caption{Stellar component matrix}
  \renewcommand{\arraystretch}{1.2}
  \small
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}p{2.8cm} X@{}}
    ...
    \midrule
    Authority contract & Soroban Rust: issuer whitelist \\
    Credentials contract & Soroban Rust: hash anchor + revocation \\
    Audit contract & Soroban Rust: event anchoring \\
    Blockchain SDK & @stellar/stellar-sdk + Soroban RPC \\
    Wallet & Freighter, Albedo, Stellar Wallet Kit \\
    Auth & Sign-in-with-Stellar (SEP-53) + JWT \\
    ZK engine & Off-chain Pedersen + sigma proofs; on-chain hash anchor \\
...
\begin{enumerate}
  ...
  \item Ed25519-native Pedersen commitments (optional crypto alignment with account keys)
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\bfseries}p{2.5cm} p{1.2cm} X X@{}}
    ...
    Ruthless scope cut; staged demo with Testnet fallback if Mainnet timing slips \\
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}ll X@{}}
    ...
    POST & \texttt{/api/auth/verify} & Wallet authentication (SIWS) \\
    \bottomrule
  \end{tabularx}
\end{table}
